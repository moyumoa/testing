...
const UsersList = () => {
    ...
    USERINFOFORM.validateFields().then((values) => {
      ...
      notify.task({
        ...
        final: async () => {
          ...
          setQueryParams((prev) => ({ ...prev, t: Date.now() })) // 触发数据刷新
  ...
  /* 删除用户 */
  const delateRole = async (id, remove) => {
    notify.task({
      text: "处理中...",
      final: async () => {
    ...
    <>
      ...
      >
        <SearchForm
          ...
          right={
            <AuthWrap permission="sgc:system:user:post">
        ...
        renderItem={(item, { remove }) => (
          ...
          >
            <Card
              ...
              actions={[
                <AuthWrap key="edit" permission="sgc:system:user:put">
                  <EditOutlined key="edit" onClick={() => showModal(item)} />
                </AuthWrap>,
                <AuthWrap
                  key="list-loadmore-more"
                  permission="sgc:system:user:put"
                >
                  <a onClick={() => changePassword(item)}>修改密码</a>
                </AuthWrap>,

                ...(item.username !== "system"
                  ? [
                      <AuthWrap key="ban" permission="sgc:system:banuser:put">
                        <Button
                          color="danger"
                          variant="link"
                      ...
                      <AuthWrap
                        key="delete"
                        permission="sgc:system:user:delete"
                      >
                        <Popconfirm
                          title="删除用户"
                          description="所有关联该用户的用户登录信息将立即失效, 是否要继续?"
                          onConfirm={() => delateRole(item._id, remove)}
                          onCancel={() => {}}
                          okText="确认删除"
                          okType="danger"
                          cancelText="取消"
      ...
      >
        ...
        >
          ...
          >
            <Input
              disabled={tempInfo?.username === "system"}
              placeholder="请输入账号"
            />
          </Form.Item>
          ...
          <Form.Item name="status" label="是否启用" required>
            <Switch
              disabled={tempInfo?.is_banned || tempInfo?.username === "system"}
            />
          </Form.Item>
        </Form>
      </Modal>
