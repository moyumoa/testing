...
export class Convert {
  static toConversation(conversationMap: any): Conversation {
    ...
    if (recents && recents.length > 0) {
      const messageModel = Convert.toMessage(recents[0]);
      conversation.lastMessage = messageModel
    }
    conversation.extra = {}
    conversation.extra.top = conversationMap["stick"]
    if (conversationMap["extra"]) {
      conversation.remoteExtra = Convert.toConversationExtra(conversation.channel, conversationMap["extra"])
    }
  ...
  static toMessage(msgMap: any): Message {
    ...
    if (msgMap["message_extra"]) {
      ...
      message.remoteExtra = Convert.toMessageExtra(messageExtra)
    }
    ...
    if (contentObj) {
      messageContent.decode(Convert.stringToUint8Array(JSON.stringify(contentObj)))
    }
  ...
  static toMessageExtra(msgExtraMap: any): MessageExtra {
    ...
    if (contentEditObj) {
      ...
      const contentEditPayloadData = Convert.stringToUint8Array(JSON.stringify(contentEditObj))
