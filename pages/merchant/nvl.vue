  ...
  <view class="pagecontainer">
    ...
    <view class="crads" v-for="(item, index) in paginated.data" :key="index" @tap="oper('detail', item)">
      <view class="crads-top">
        <text class="crads-top-lt" :style="{ color: `${1 === 1 ? '#749af8' : '#e67378'}` }"> {{ item.authStatusTxt
        }}</text>
      ...
      <view class="crads-bottom">
        ...
        <view class="crads-bottom-rtbtn" @tap="oper('edit', item)">去修改</view>
      </view>
...
export default {
  ...
  async onLoad () {
    this.paginated = createPaginated(uni.$api.authListShop, { userId: this.getUserInfo.id }, {
      pageKey: 'pageNum',
      limitKey: 'pageSize',
