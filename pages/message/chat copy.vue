<template>
  <view>
    <!-- :style="{ height: `calc(var(--real-height) - ${inputHeight}px - env(safe-area-inset-bottom))` }" -->
    <view class="chat-page"
      :style="{ height: `calc(100vh - ${inputHeight}px - env(safe-area-inset-bottom) - env(safe-area-inset-top))` }">
        ...
        <scroll-view scroll-y enable-flex class="scroll-list" @scrolltolower="loadHistory" :scroll-top="scrollTop">
          <view class="message-item-wrap">
            <view v-for="(item, index) in messageList" :key="item.key" @click="onClick(index)">
              <MessageItem :detail="item.payload"></MessageItem> {{ inputHeight }}
            </view>
          </view>
    ...
    <!-- <view id="chat-input-wrap" class="input-wrap" :style="{ bottom: inputBottomValue }">
      <textarea v-model="inputValue" class="input-area"  :auto-height="true" :adjust-position="false" rows="1"
        @confirm="sendMessage" />
      <view class="send-btn" @tap="sendMessage">
        <image src="@/static/enter_n.png" mode=""></image>
      </view>
  ...
  onReady () {
    ...
    this.$nextTick(() => {
      ...
      query.select('#chat-input-wrap').boundingClientRect(data => {
        console.log('ref inputRef 的高度为:', data.height)
