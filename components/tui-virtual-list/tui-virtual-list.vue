<template>
	<view class="tui-virtual-list" :style="{ background: background }">
		<scroll-view :style="getStyle" class="tui-virtual__scroll-view" :scroll-y="!enablePageScroll" :show-scrollbar="false"
			:upper-threshold="upperThreshold" :lower-threshold="lowerThreshold" :scroll-top="innerScrollOffset"
			:scroll-with-animation="scrollWithAnimation" :enable-back-to-top="enableBackToTop"
			@scroll="onScroll"
			@scrolltoupper="onScrollToUpper" @scrolltolower="onScrollToLower">
			<!-- :refresher-enabled="false"
			:refresher-threshold="50" :refresher-triggered="false" @refresherrefresh="onRefresh" -->
			<view :style="styles">
				<slot></slot>
			</view>
		</scroll-view>
...
export default {
	...
	created () {
		...
		this.cellHeight = this.itemHeight;
		this.$emit('init', {
	...
	methods: {
		onRefresh (e) {
			this.$emit('refresherrefresh', e.detail)
		},
			getPx (rpx) {
