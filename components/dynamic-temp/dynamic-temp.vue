<template>
  <view class="blocktgrid-items">
      ...
      <view class="uoverview-c">
        ...
        <view class="uoverview-c-bottom">
          <view style="display: flex; align-items: center;">
            <template v-if="item.appUserDynamic.addHidden">
              <u-icon name="map" color="#333" size="12" style="margin-right: 2px;" />
              <text class="uoverview-c-bottom-t">{{ item.appUserDynamic.cityAddress }}</text>
            </template>
            <text class="uoverview-c-bottom-t">{{ formatSmartTime(item.appUserDynamic.createTime) }} 发布</text>
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="description">
      <text>{{ item.appUserDynamic.description }}</text>
    </view>

    <view class="cell-item-several">
      <view class="cell-item-pic" v-if="item.appUserDynamic.videoUrl" @tap="previewVideo(item.appUserDynamic.videoUrl)">
        <video :src="item.appUserDynamic.videoUrl" />
      </view>
      <template v-else>
    ...
    <view class="blocktgrid-bottom">
      <view class="blocktgrid-bottom-l">
      </view>
      <view class="blocktgrid-bottom-r">
        <u-icon class="blocktgrid-bottom-r-icon" name="eye" :label="formatNum(item.appUserDynamic.viewCount) || 0"
          size="15" labelColor="#222" labelSize="12" v-if="false" />
        <u-icon class="blocktgrid-bottom-r-icon" name="heart" :label="formatNum(item.appUserDynamic.praiseCount) || 0"
          size="15" labelColor="#222" labelSize="12" />
        <u-icon class="blocktgrid-bottom-r-icon" name="chat" :label="formatNum(item.appUserDynamic.reviewCount) || 0"
          size="15" labelColor="#222" labelSize="12" v-if="false" />
      </view>
    </view>
