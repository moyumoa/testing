	...
	<view class="u-tabs" :style="{
		...
		<scroll-view scroll-x class="u-scroll-view" :scroll-left="scrollLeft" scroll-with-animation :style="{ zIndex: zIndex + 1 }">
			<view class="u-tabs-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
				<view class="u-tabs-item" :id="`tab${index}`" :style="[tabItemStyle(index)]"
				 v-for="(item, index) in getTabs" :key="index" :class="[preId + index]" @tap="emit(index,item)">
			...
			tabItemStyle() {
				return (index) => {
					let style = {
						...
						// fontSize: (index == this.getCurrent) ? `${this.fontSize+2}px` : `${this.fontSize}px`,
